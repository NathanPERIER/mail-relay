...
def main():
    ...
    controller = Controller(conf.handler, loop=loop, hostname=conf.smtp_host, port=conf.smtp_port, authenticator=conf.auth, auth_required=(conf.auth is not None), auth_requires_tls=False)
    controller.begin()
